...
def load_vector_db(session_id, user_email):
    """
    ...
    """
    cached_index = redis_client.get(f"{session_id}_index")
    cached_metadata = redis_client.get(f"{session_id}_metadata")
    if cached_index and cached_metadata:
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                # Write the index and metadata to files
                index_path = os.path.join(temp_dir, 'index.faiss')
                metadata_path = os.path.join(temp_dir, 'index.pkl')

                with open(index_path, 'wb') as f:
                    f.write(cached_index)

                with open(metadata_path, 'wb') as f:
                    f.write(cached_metadata)

                # Load the vector store from the files
                vectorstore = FAISS.load_local(temp_dir, OpenAIEmbeddings(), allow_dangerous_deserialization=True)
                logger.info(f"Loaded vectorstore from cache for session_id: {session_id}")
                return vectorstore
        except Exception as e:
            logger.error(f"Failed to load vectorstore from cache for session_id: {session_id}: {e}")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            try:
                ...
                db_path = temp_dir
                if not (os.path.exists(os.path.join(db_path, 'index.faiss')) and os.path.exists(os.path.join(db_path, 'index.pkl'))):
                ...
                redis_client.setex(f"{session_id}_index", 3600, index_data.tobytes())  # Cache with TTL of 1 hour
                redis_client.setex(f"{session_id}_metadata", 3600, metadata)  # Cache with TTL of 1 hour

                logger.info(f"Loaded vectorstore from GCS and cached for session_id: {session_id}")
                return vectorstore
            except GoogleCloudError as e:
                logger.error(f"Failed to download vectorstore from GCS for session_id: {session_id}: {e}")
                raise RuntimeError("Failed to download vectorstore from GCS") from e
            except Exception as e:
                logger.error(f"Failed to load vectorstore for session_id: {session_id}: {e}")
